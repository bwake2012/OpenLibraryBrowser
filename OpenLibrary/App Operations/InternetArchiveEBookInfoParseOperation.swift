...
class InternetArchiveEbookInfoParseOperation: Operation {
    ...
    /**
        ...
        - parameter context: The `NSManagedObjectContext` that will be used as the
                             ...
                             passed-in context.
    */
    init( workKey: String, editionKey: String, cacheFile: NSURL, coreDataStack: CoreDataStack, updateResults: SearchResultsUpdater ) {
    ...
    override func execute() {
        ...
        do {
            if let json = try NSJSONSerialization.JSONObjectWithStream(stream, options: []) as? [[String: AnyObject]] {
                ...
                updateResults( SearchResults( start: 0, numFound: 0, pageSize: 0 ) )
                finish()
            }
        }
    ...
    private func parse( resultSet: [[String: AnyObject]] ) {
        ...
        context.performBlock {
            ...
            for entry in resultSet {
                ...
                if let newObject = OLLanguage.parseJSON( sequence, index: index, match: entry, moc: self.context ) {
...
//                    print( "\(newObject.key) \(newObject.code) \(newObject.name)" )
            ...
            if nil == error {
                self.updateResults( SearchResults( start: 0, numFound: numFound, pageSize: resultSet.count ) )
            }
